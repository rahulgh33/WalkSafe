...
locations = [
    ...
    ("Scottsdale",           41.9500, -87.7700),  # (Note: Scottsdale here is a Chicago neighborhood name; do verify as needed.)
    ...
    ("The Island",           41.8780, -87.6300),  # (Placeholder coordinate—verify actual location if needed.)
...
# --- Main loop ---
print(f"{'Location':<20} {'CrimeCount':<12} {'WeightedSum':<14} {'POIs':<6} {'LitPOIs':<9} {'HelpDist':<10}")
print("-" * 70)
...
for name, lat, lon in locations:
    ...
    for crime in crime_data:
        ...
        if ctype in CRIME_WEIGHTS:
            ...
            if ctype in ["BATTERY", "ASSAULT"] and "AGGRAVATED" not in description:
                weight *= 0.5
            weighted_sum += weight
            incident_count += 1
    ...
    try:
        ...
        print(f"⚠️ No POIs found near {name}")
    ...
    try:
        ...
        print(f"⚠️ No emergency services near {name}")
        dist_to_help = MAX_HELP_DISTANCE
    else:
        dist_to_help = get_min_distance(lat, lon, emergency_gdf)
    help_score = min(dist_to_help / MAX_HELP_DISTANCE, 1.0)

    # Step 5: Save raw features
    data.append({
        "Location": name,
        "CrimeCount": incident_count,
        "WeightedCrimeSum": round(weighted_sum, 2),
        "NumPOIs": poi_count,
        "NumLitPOIs": lit_poi_count,
        "LightingRatio": round(lit_poi_count / (poi_count + 1), 2),
        "DistToHelp": dist_to_help,
        "HelpScore": round(help_score, 2)
    })
    print(f"{name:<20} {incident_count:<12} {weighted_sum:<14.2f} {poi_count:<6} {lit_poi_count:<9} {dist_to_help:.1f}")

# Export CSV
df = pd.DataFrame(data)
df.to_csv("features_raw.csv", index=False)
print("✅ Raw feature data saved to features_raw.csv")
